...
#%%
svgName = 'figureS10QLearning.svg'
...
for r in rewards:
    fancyViz.drawWaterDrop(ax, np.array([r, 6]), np.array([50, 1]),
                           facecolor='k')
    ax.axvline(r, .05, .85, lw=mpl.rcParams['axes.linewidth'], ls='--', color='k')
ax.axhline(0, ls=':', color='k', lw=mpl.rcParams['axes.linewidth'], zorder=-1)
ax.hlines(-7, 18800, 20000, clip_on=False, color='k', lw=mpl.rcParams['axes.linewidth'])
#ax.text(19400, -8, "1 min".format(1000/20), ha="center", va="top", fontsize=6)
ax.set_yticks((-5,0,5))
ax.set_yticks((-2.5,2.5), minor=True)
...
ax.plot(xx, 100*fracRight, ".-", clip_on=False)
ax.axhline(50, ls=':', color='k', lw=mpl.rcParams['axes.linewidth'], zorder=-99)
...
for p in ("alpha", "beta", "bias"):
    ...
    sns.swarmplot(y=params[p], ax=ax, size=2, clip_on=False)
    ax.set_ylabel("")
    ...
    ax.set_title(p)
    if p == 'bias':
        ax.axhline(0, ls=':', color='k', lw=mpl.rcParams['axes.linewidth'], zorder=-99)
...
sns.lineplot(qAVs.Q_actionValue[mask].round(0), regAVs.value[mask], sort=True,
             ...
             markeredgecolor="C0")
ax.plot([-7,7],[-7,7], color='k', ls=':', lw=mpl.rcParams['axes.linewidth'], zorder=-99)
...
#           lw=0, alpha=.5, zorder=1, width=.5)
    ...
    ax.axhline(0, ls=':', color='k', alpha=1, lw=mpl.rcParams['axes.linewidth'], clip_on=False)
